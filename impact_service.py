...
def calculate_impact(ecosystem_type, area_hectares, annual_value_inr,
    ...
    if water_value > 0:
        ...
        impacts.append({"icon":"💧","color":"#3a9bd5","metric":_fmt(people),
            "label":"people supplied clean water","sub":f"Water services worth ₹{_fmt(water_value)}/yr"})
    ...
    impacts.append({"icon":"🌳","color":"#52b36b","metric":_fmt(trees),
        "label":"mature trees equivalent","sub":f"= {_fmt(solar)} solar panels worth of carbon offset"})
    ...
    if flood_value > 0:
        ...
        impacts.append({"icon":"🛡️","color":"#f0b429","metric":_fmt(people),
            "label":"people protected from floods","sub":f"Avoided damage: ₹{_fmt(flood_value)}/yr"})
    ...
    if n > 0:
        impacts.append({"icon":"👨‍👩‍👧","color":"#b36bd4","metric":_fmt(n),
            "label":kind,"sub":"directly supported by this ecosystem"})

    if biodiversity_index >= 70:
        impacts.append({"icon":"🦋","color":"#e05c8a","metric":_fmt(int(area_hectares*0.12)),
            "label":"wildlife species habitat","sub":f"Biodiversity Index: {biodiversity_index}/100"})

    if climate_score >= 60:
        impacts.append({"icon":"🌡️","color":"#52b36b","metric":str(max(1,int(area_hectares/50))),
            "label":"villages climate-buffered","sub":f"Climate Resilience Score: {climate_score}/100"})

    impacts.append({"icon":"⚠️","color":"#e05c2a","metric":f"₹{_fmt(annual_value_inr*6.71)}",
        "label":"lost forever if developed","sub":"10-year ecosystem NPV that cannot be recovered"})
